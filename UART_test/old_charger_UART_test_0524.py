...
ptu_read_after_pru_left = PXU_Readings()
...
def detect_restart_command():
	...
	while(pbs_restart_received != 1):
		a_response = get_a_valid_command()
		if a_response == []:
...
# return 1: error
#***************************************
def detect_charge_command_and_pru_addr(address):
	...
	while(pbs_charge_start_received != 1 or pru_charge_start_received != 1):
		...
		if(a_response[2] == PBS_CHARGE and a_response[8] == START_CHARGE):
			...
			print()
		if(a_response[2] == PRU_CHARGE_START):
...
def main():
	...
	pru_address = []
	result = detect_charge_command_and_pru_addr(pru_address)
	if result == 1:
		print_test_fail()
		ComPort.close()
		return
	'''
	print('PRU address found: ', end="")
	for i in range(0, 6):
		print(format(pru_address[i], '02X'), end="")
		print(' ', end="")
	'''
	#-------------------------------------------------------
	wait_time = 2	# second, for readings to get stable
	time.sleep(wait_time)
