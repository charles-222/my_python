...
for_ENE_test		= 0		# test ENE product ********************************
...
#		1: error
#*********************************************
def get_ptu_readings_on_lock(command):
	#wait_time = 1	# second
	return_data = []
	error = 0
	if(command == ASK_OFF):
...
def main():
	...
	while user_continue == 1:
		...
		print('==========>>> Please power on PBS now \n')	# -- to detect Restart and read Versions and PTU readings \n')
		#'''
		print('-------- now detect PBS Restart command')
		...
		print('==========>>> Please put PRU on charge pad \n')	#-- to test Charge Start and read PTU & PRU readings \n')
		pru_address = []
		detect_prx_address = []
		print('-------- now detect PTU and PRU Start Charge commands')
		...
		print('-------- now test PTU and PRU reading commands')
		...
		print('-------- now test Off, On, Lock and Unlock commands')
		error += get_ptu_readings_on_lock(ASK_OFF)		# test Off Off Off Off Off Off
		result = detect_stop_charge_command(pru_address)
		if result == 1:
			user_continue = detect_failure_process(fout)
			continue

		error += get_ptu_readings_on_lock(ASK_ON)		# test On On On On On On On On
		pru_address = []
		detect_prx_address = []
		result = detect_start_charge_command_and_pru_addr(pru_address, detect_prx_address)
		if result == 1:
		...
		error += get_ptu_readings_on_lock(ASK_LOCK)	# test Lock Lock Lock Lock Lock
		result = detect_stop_charge_command(pru_address)
		if result == 1:
			user_continue = detect_failure_process(fout)
			continue
		...
		error += get_ptu_readings_on_lock(ASK_UNLOCK)		# test UnLock UnLock UnLock UnLock
		pru_address = []
		detect_prx_address = []
		result = detect_start_charge_command_and_pru_addr(pru_address, detect_prx_address)
		if result == 1:
		...
		print('-------- now test PRx ID command')
		error += get_ptu_readings_pru_id()	# test Lock Lock Lock Lock Lock
		result = detect_pru_id_command(pru_address)
		if result == 1:
			user_continue = detect_failure_process(fout)
			continue
		
		print('==========>>> Please remove PRU from charge pad \n')	#-- to test Charge Stop and Not Found responses \n')
		
		print('-------- now detect PTU and PRU Stop Charge commands')
		result = detect_stop_charge_command(pru_address)
		if result == 1:
			user_continue = detect_failure_process(fout)
			continue
