...
def main():
	...
	if 1:
		...
		#--------------------------------------------------------------------
		on_off_on_1 = []
		result = get_a_resource('PTU GET PBS ON_OFF', '/3300/1/5850?change=1', on_off_on_1)	# with or without / in front of ? are both OK for Californium
		if result:	return

		result = put_a_resource('PTU PUT PBS OFF', '/3300/1/5850 0')	# 0 is OFF
		if result:	return

		on_off_off = []
		result = get_a_resource('PTU GET PBS ON_OFF', '/3300/1/5850?change=1', on_off_off)	# with or without / in front of ? are both OK for Californium
		if result:	return

		result = put_a_resource('PTU PUT PBS ON', '/3300/1/5850 1')		# 1 is ON
		if result:	return

		on_off_on_2 = []
		result = get_a_resource('PTU GET PBS ON_OFF', '/3300/1/5850?change=1', on_off_on_2)	# with or without / in front of ? are both OK for Californium
		if result:	return

		#--------------------------------------------------------------------
		lock_unlock_unlock_1 = []
		result = get_a_resource('PTU GET PBS Lock_UnLock', '/3300/2/5850?change=1', lock_unlock_unlock_1)
		if result:	return

		result = put_a_resource('PTU PUT PBS Lock', '/3300/2/5850 1')	# 1 is Lock
		if result:	return

		lock_unlock_lock = []
		result = get_a_resource('PTU GET PBS Lock_UnLock', '/3300/2/5850?change=1', lock_unlock_lock)
		if result:	return

		result = put_a_resource('PTU PUT PBS UnLock', '/3300/2/5850 0')	# 0 is UnLock
		if result:	return

		lock_unlock_unlock_2 = []
		result = get_a_resource('PTU GET PBS Lock_UnLock', '/3300/2/5850?change=1', lock_unlock_unlock_2)
		if result:	return
